...
def makeAppDir(event):
	try:
		os.mkdir(os.path.expanduser("~/LWFiles"))
	except OSError:
		comm_line.insert(3, " | Failed creating directory")
	else:
		comm_line.insert(3, " | Successfully created app directory")
...
app.bind('<Control-Shift-A>', makeAppDir)
